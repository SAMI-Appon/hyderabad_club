...
function pos_boot($ul, $pt, $lc, $em, $un, $type = 1, $pid = null)
{
    $ch = curl_init();
    $request_url =
        $type == 1 ? base64_decode(config('author.lic1')) : base64_decode(config('author.lic2'));
    ...
    if ($result) {
        ...
        if ($result['flag'] == 'valid') {
            ...
            $msg =
                isset($result['msg']) && !empty($result['msg'])
                    ? $result['msg']
                    : 'I' . 'nvali' . 'd ' . 'Lic' . 'ense Det' . 'ails';
            return redirect()
                ->back()
                ->with('error', $msg);
        }
...
function isPusherEnabled()
{
    ...
    if (
        ...
        !empty(config('broadcasting.connections.pusher.options.cluster')) &&
        config('broadcasting.connections.pusher.driver') == 'pusher'
    ) {
...
if (!function_exists('expense')) {
    function expense($expense = true)
    {
        $output = ($expense) ? 'Expense' : 'Sale';
        return $output;
    }
}
